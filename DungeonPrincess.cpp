...
int dungeon(vector<vector<int>> &A, int i, int j, vector<vector<int>> &dp){
    if(i==A.size()-1 && j==A[0].size()-1)   return max(1, 1-A[i][j]);
    if(i==A.size()) return max(1, dungeon(A, i, j+1, dp)-A[i][j]);
    if(j==A[0].size()-1)    return max(1, dungeon(A, i+1, j, dp)-A[i][j]);
    if(dp[i][j]!=2e7)   return dp[i][j];
    dp[i][j]=max(1, min(dungeon(A, i+1, j, dp), dungeon(A, i, j+1, dp))-A[i][j]);
    return dp[i][j];
}

int Solution::solve(vector<vector<int>> &A){
    vector<vector<int>> dp(A.size(), vector<int> (A[0].size(), 2e7));
    return dungeon(A, 0, 0, dp);
}