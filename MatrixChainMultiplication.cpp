...
int matMul(vector<int> &A, int i, int j, vector<vector<int>> &dp){
    if(i==j)    return 0;
    if(dp[i][j]!=-1)    return dp[i][j];
    dp[i][j]=INT_MAX;
    for(int k=i;k<j;k++){
        dp[i][j]=m,in(dp[i][j], matMul(A, i, k, dp)+matMul(A, k+1, j, dp)+(A[i-1]*A[k]*A[j]));
    }
    return dp[i][j];
}
